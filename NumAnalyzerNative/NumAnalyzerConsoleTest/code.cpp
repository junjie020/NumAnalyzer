...
std::wstring CDecodeTextFile::DecodeFileToWstring(const wchar_t *filePath)
{
    TextCodeType type = GetFileEncodeType(filePath);
    return ReadString(type);
}

std::string CDecodeTextFile::DecodeFileToString(const wchar_t *filePath)
{
    std::string Dst = "";
    return Dst;
}
...
std::wstring CDecodeTextFile::ReadString(TextCodeType type)
{
    if (!m_fp) return NULL;
    switch (type)
    {
...
std::wstring CDecodeTextFile::ReadAnsiString()
{
    const char* GBK_LOCALE_NAME = ".936"; //GBK在windows下的locale name

    std::wstring_convert<std::codecvt_byname<wchar_t, char, mbstate_t>> Conver_GBK(new std::codecvt_byname<wchar_t, char, mbstate_t>(GBK_LOCALE_NAME));    //GBK - whar
    ...
    std::wstring wDst = Conver_GBK.from_bytes(buf);        //GBK转Unicode

    delete buf;
    return wDst;
}
std::wstring CDecodeTextFile::ReadUtf8String()
{
    ...
    std::wstring wDst = Conver_UTF8.from_bytes(buf);        //GBK转Unicode

    delete buf;
    return wDst;

}
std::wstring CDecodeTextFile::ReadUnicodeString()
{
    fseek(m_fp, 0, SEEK_END);
    int fileSize = ftell(m_fp);
    ...
    std::wstring wDst = buf;

    delete buf;
    return wDst;
}
std::wstring CDecodeTextFile::ReadUnicodeBigString()
{
    std::wstring wDst= L"";
    return wDst;
}
